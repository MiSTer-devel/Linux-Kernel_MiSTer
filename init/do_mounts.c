...
static void __init mount_block_root(char *root_device_name)
{
	...
	if (loop_name) {
		...
		if (err)
			pr_emerg("Failed to mount /dev/root as VFAT or exFAT: %d\n", err);
		...
		mount_root_generic("/dev/loop8", root_device_name, root_mountflags);

		err = init_mount("/root2", "/root/media/fat", "", MS_BIND, "");
		if (err)
			pr_emerg("Failed to bind-mount /root2 to /root/media/fat: %d\n", err);
	} else {
