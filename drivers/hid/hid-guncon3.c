 ...
 * USB GunCon3 Driver
 *
 * Copyright (C) Beardypig <guncon@beardypig.com>
 *
...
static const struct usb_device_id usb_guncon3_id_table[] = {
	{ USB_DEVICE(0x0b9a, 0x0800) },
	{ } /* Terminating entry */
};
...
static int guncon3_decode(unsigned char *data, const unsigned char *key)
{
	...
	if (a_sum != key[7]) {
		if (debug)
			pr_err("guncon3: checksum mismatch: %02x %02x\n", a_sum, key[7]);
		return -1;
	}
...
static void usb_guncon3_irq(struct urb *urb)
{
	...
	if (status != 0) {
		/* TODO: submit new key */
		pr_err("guncon3: checksum error\n");
		goto resubmit;
	}

	if (debug) {
		pr_info("guncon3_debug: data :");
		for (i = 0; i < 13; i++)
...
resubmit:
	...
	if (status)
		pr_err("guncon3: can't resubmit intr, %s-%s/input0, status %d",
...
static void guncon3_irq_in(struct urb *urb)
{
	...
	pr_err("guncon3: could not send key packet\n");
}
...
static int usb_guncon3_open(struct input_dev *dev)
{
	...
	status = usb_submit_urb(guncon3->irq_out, GFP_KERNEL);
	if (status) {
		if (debug)
			pr_err("guncon3: %s open input urb failed: %d\n", guncon3->phys, status);
		guncon3->open--;
		return -EIO;
	}

	if (debug)
		pr_info("guncon3: open successful\n");
...
static int guncon3_init_output(struct usb_interface *intf, struct usb_guncon3 *guncon3)
{
	...
	maxp = usb_maxpacket(guncon3->usbdev, guncon3->opipe);
...
fail1:
	pr_err("guncon3: error while setting up output irq\n");
	return error;
}
...
static int guncon3_init_input(struct usb_interface *intf, struct usb_guncon3 *guncon3)
{
	...
	maxp = usb_maxpacket(guncon3->usbdev, guncon3->ipipe);
...
fail1:
	pr_err("guncon3: error while setting up input irq\n");
...
static int guncon3_send_key(struct usb_guncon3 *guncon3)
{
	...
	if (debug)
		pr_info("guncon3: sending init packet to guncon...\n");
...
static int usb_guncon3_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	guncon3 = kzalloc(sizeof(*guncon3), GFP_KERNEL);
	if (!guncon3)
		return -ENOMEM;
	...
	strscpy(guncon3->name, "guncon3", sizeof(guncon3->name));
	...
	if (debug)
		pr_info("guncon3: input: %s registered, error %d\n", guncon3->name, error);
...
err_free_guncon:
	kfree(guncon3);
	pr_err("guncon3: error while setting up device\n");
	return error;
}
...
module_usb_driver(usb_guncon3_driver);
