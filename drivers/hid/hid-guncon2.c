...
static void guncon2_usb_irq(struct urb *urb) {
    ...
    if (urb->actual_length == 6) {
        ...
        // main buttons
        input_report_key(guncon2->input_device, BTN_X, buttons & GUNCON2_TRIGGER);
        ...
        input_report_key(guncon2->input_device, BTN_Y, buttons & GUNCON2_BTN_C);
...
static int guncon2_probe(struct usb_interface *intf,
   ...
   */
    ...
    input_set_capability(guncon2->input_device, EV_KEY, BTN_Y);
...
static const struct usb_device_id guncon2_table[] = {
        {USB_DEVICE(USB_VENDOR_ID_NAMCO, USB_PRODUCT_ID_NAMCO_GUNCON2)},
        {}};
