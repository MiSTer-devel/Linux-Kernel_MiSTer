...
int gip_auth_get_transcript(struct shash_desc *desc, void *transcript)
{
	...
	/* FORWARD-PORT BUG FIX: the original 6.12+ branch returns the local
	 * `err` here uninitialized on the allocation-failure path (never
	 * assigned before this point) -- returns garbage instead of -ENOMEM.
	 * Not a version-adaptation artifact, a pre-existing bug in this
	 * branch's own upstream source; fixed per this repo's established
	 * policy of fixing real bugs found while porting rather than
	 * carrying them silently. */
