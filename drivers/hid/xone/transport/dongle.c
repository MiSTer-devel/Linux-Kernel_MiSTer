...
/* FORWARD-PORT: struct usbdrv_wrap/the .drvwrap field are gone entirely
 * (include/linux/usb.h) -- struct usb_driver now embeds `struct
 * device_driver driver` directly, and separately has gained a proper
 * top-level `void (*shutdown)(struct usb_interface *intf)` field that did
 * not exist as public API when this driver was written (which is why the
 * original reached into the low-level device_driver hook in the first
 * place). Traced the real dispatch path in this tree
 * (drivers/usb/core/driver.c: usb_register_driver() unconditionally sets
 * new_driver->driver.shutdown = usb_shutdown_interface at registration
 * time, which would silently clobber a .driver.shutdown assignment before
 * any real shutdown event could reach it; usb_shutdown_interface() itself
 * looks up the top-level usb_driver.shutdown field) -- re-anchored to the
 * hook that is provably live on this tree, changing the parameter type to
 * match (struct usb_interface * instead of struct device *, dropping the
 * now-redundant to_usb_interface() conversion). Logic/behavior otherwise
