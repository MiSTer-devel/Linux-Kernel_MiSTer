...
/*
 * HID driver for Fanatec wheels (CSL Elite / CSR Elite / ClubSport / Podium)
 *
...
/* adjustable initial value for brake load cell */
static int init_load = 4;
...
static int ftec_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (drv_data->quirks & FTEC_PEDALS) {
		...
		/* if these bits are not set, the pedals are not recognized in newer proton/wine versions */
