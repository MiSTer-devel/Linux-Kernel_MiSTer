...
static void gamecube_adpt_destroy(struct gamecube_adpt *adpt)
{
	...
	/*
	 * BUG FIX (not present in the 5.15 MiSTer fork; see the patch header).
	 *
	 * The original teardown cancelled only work_rumble and left the four
	 * per-port work_connect items dangling, then kfree()d the adapter.
	 * work_connect is embedded in adpt->ctrls[], and its handler
	 * container_of()s back to the ctrl and dereferences ctrl->adpt->hdev --
	 * so an item still pending when the kfree() below ran was a
	 * use-after-free on unplug.
	 *
	 * The ordering here is load-bearing, not cosmetic:
	 *
	 * 1. Stop the transport FIRST. gamecube_ctrl_update_flags() runs from
	 *    the HID report callback and is the only thing that schedules
	 *    work_connect; once hid_hw_stop() has returned, no further report --
	 *    and therefore no further work_connect -- can be scheduled.
	 *    Cancelling before the stop would race: a report arriving between
	 *    the cancel and the stop simply re-arms the work.
	 */
	hid_hw_close(adpt->hdev);
	hid_hw_stop(adpt->hdev);

	/*
	 * 2. Drain work_connect BEFORE unregistering the inputs, deliberately.
	 *    A still-pending work_connect may call gamecube_ctrl_create(), and
	 *    any input device it registers must then be torn down by the loop
	 *    below. Draining after the loop would leak that device -- and leave
	 *    it pointing at an adapter we are about to free.
	 */
	for (i = 0; i < 4; i++)
		cancel_work_sync(&adpt->ctrls[i].work_connect);

	/*
	 * 3. Unregistering the inputs closes the userspace force-feedback path,
	 *    which is the only thing that schedules work_rumble. (A rumble that
	 *    slips through between the hid_hw_stop() above and here is harmless:
	 *    hid_hw_output_report() on a stopped device returns -ENOSYS.)
	 */
	for (i = 0; i < 4; i++)
...
static int gamecube_fixup_urb_in(struct gamecube_adpt *adpt)
{
	...
	/*
	 * 6.18 forward-port: hid_is_using_ll_driver(hdev, &usb_hid_driver) is
	 * gone -- usb_hid_driver became `static` inside usbhid/hid-core.c
	 * (no longer reachable from another module) when the USB-specific
	 * hid_is_usb() helper was added (upstream commit c122c07f470a4,
	 * v5.2) to replace open-coded ll_driver pointer comparisons.
	 */
	if (!hid_is_usb(hdev))
		return -EINVAL;
...
err:
	/*
	 * BUG FIX (same defect as in gamecube_adpt_destroy(), and equally
	 * absent from the 5.15 fork). The err_close path is only reachable
	 * AFTER hid_hw_open() has succeeded, so reports are already flowing and
	 * may have scheduled work_connect -- which is embedded in *adpt and
	 * would then run against freed memory.
	 *
	 * hid_hw_stop() above has already guaranteed no further report can
	 * schedule it, so this drain is final. cancel_work_sync() on a work
	 * item that was INIT_WORK()'d but never scheduled is a no-op, and
	 * gamecube_adpt_create() initialises all four unconditionally, so this
	 * is safe on every path into this label.
	 */
	for (i = 0; i < 4; i++)
		cancel_work_sync(&adpt->ctrls[i].work_connect);
	kfree(adpt);
