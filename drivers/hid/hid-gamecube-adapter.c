...
/*
 * Forward-port note (Track B, not in the original 5.15-era fork commit
 * 77862a67f): the original driver only ever called kfree(adpt) -- both here
 * and on every probe() error path below -- without first quiescing the four
 * per-port ctrl->work_connect work items. Each of those is scheduled
 * asynchronously from gamecube_hid_event()/gamecube_ctrl_handle_report()
 * every time a physical GameCube controller is plugged/unplugged from the
 * adapter. If a work_connect for a given port is still queued (or running)
 * at the moment the adapter itself is unplugged/removed, or at the moment
 * probe() aborts after hid_hw_open() has already started delivering input
 * reports, the work callback (gamecube_work_connect_cb) dereferences
 * ctrl->adpt and ctrl->flags_lock/ctrl->input out of memory that kfree()
 * has already released -- a use-after-free, corrupting the slab. This is
 * documented as bug B1 in Buildroot_MiSTer/PLAN.md 4.1's bug table, which
 * recommends fixing it rather than carrying it forward. Fixed by
 * cancel_work_sync()-ing every port's work_connect before the adapter
 * (and the ctrl structs embedded in it) are freed, both in the normal
 * teardown path (gamecube_adpt_destroy(), used by remove()) and in
 * probe()'s error path below.
 */
static void gamecube_ctrls_cancel_work(struct gamecube_adpt *adpt)
{
	unsigned int i;
...
static void gamecube_adpt_destroy(struct gamecube_adpt *adpt)
{
	...
	gamecube_ctrls_cancel_work(adpt);
	for (i = 0; i < 4; i++)
...
static int gamecube_fixup_urb_in(struct gamecube_adpt *adpt)
{
	...
	/*
	 * Forward-port note: hid_is_using_ll_driver(hdev, &usb_hid_driver)
	 * was removed from the kernel (no more per-ll_driver identity check);
	 * hid_is_usb() is the current equivalent used throughout drivers/hid
	 * for "is this hid_device attached via the usbhid low-level driver".
	 */
	if (!hid_is_usb(hdev))
		return -EINVAL;
...
err:
	/*
	 * See the UAF forward-port note above gamecube_adpt_destroy(): by
	 * this point hid_hw_open() may already have delivered input reports
	 * and scheduled one or more ctrl->work_connect instances before the
	 * failure above was detected, so those must be quiesced before
	 * kfree(adpt) runs (cancel_work_sync() on work that was never
	 * queued is a cheap, safe no-op).
	 */
	gamecube_ctrls_cancel_work(adpt);
	kfree(adpt);
	return ret;
