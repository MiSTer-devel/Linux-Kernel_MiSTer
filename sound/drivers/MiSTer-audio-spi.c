...
static int device_open(struct inode *inode, struct file *file)
{
	 ...
	 *
	 * The original fell through on SPI failure and computed `len` anyway.
	 * That is wrong in a way that only bites when SPI is broken -- i.e.
	 * exactly when you are reading this diagnostic to find out why:
	 *
	 *   rptr stays -1;  -1 >> 8 is still -1 (arithmetic shift);
	 *   the `(unsigned int)rptr` cast below then compares it as 0xffffffff,
	 *   so `MrBufferInfo.ptr < (unsigned)rptr` is always true and the
	 *   ring-wraparound branch is always taken, yielding
	 *       len = ptr + MrBufferInfo.len + 1
	 *   -- a "length" larger than the entire ring buffer.
	 *
	 * There is no memory-safety issue (msg[] is 1024 bytes and the format
	 * cannot overflow it), and nothing in userland parses this string --
	 * asound.conf only *writes* to /dev/MrAudio, and Main_MiSTer contains no
	 * ALSA code at all -- so the text is free to change. Report the failure
	 * instead of inventing a number.
	 */
	if (spi_read(g_spi, &rptr, 4)) {
		sprintf(msg, "rptr: <SPI read failed>, wptr: %6u, len: <unknown>, comp: -\n",
			MrBufferInfo.ptr);
		msg_Ptr = msg;
	...
	sprintf(msg, "rptr: %6d, wptr: %6u, len: %6d, comp: %1d\n", rptr, MrBufferInfo.ptr, len, comp);
...
static void cleanup(int device_created)
{
	...
	myclass = NULL;
	if (major != (dev_t)-1) unregister_chrdev_region(major, 1);
	major = -1;
...
static int device_init(void)
{
	...
	// ls /sys/class
	myclass = class_create(DRIVER_NAME "_sys");
	if (IS_ERR(myclass))
	{
	...
	// ls /dev/
	if (IS_ERR(device_create(myclass, NULL, major, NULL, DRIVER_NAME)))
	{
...
static void device_remove(struct spi_device *spi)
{
	cleanup(1);
}
